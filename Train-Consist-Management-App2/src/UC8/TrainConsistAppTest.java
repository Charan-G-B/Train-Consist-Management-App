...
public class TrainConsistAppTest {
    ...
    private List<TrainConsistApp.Bogie> bogies;

    @BeforeEach
    void setUp() {
        // Initialize the standard list used in UC7/UC8
        bogies = new ArrayList<>();
        bogies.add(new TrainConsistApp.Bogie("Sleeper", 72));
        bogies.add(new TrainConsistApp.Bogie("AC Chair", 56));
        bogies.add(new TrainConsistApp.Bogie("First Class", 24));
        bogies.add(new TrainConsistApp.Bogie("General", 90));
    }

    @Test
    void testFilter_CapacityGreaterThanThreshold() {
        // Tests: Bogies with capacity greater than 70 appear in the result
        List<TrainConsistApp.Bogie> result = bogies.stream()
    ...
    void testFilter_CapacityEqualToThreshold() {
        ...
        bogies.add(new TrainConsistApp.Bogie("Special", 70));
        List<TrainConsistApp.Bogie> result = bogies.stream()
                .filter(b -> b.capacity > 70)
                .collect(Collectors.toList());
    ...
    void testFilter_EmptyBogieList() {
        ...
        List<TrainConsistApp.Bogie> emptyList = new ArrayList<>();
        List<TrainConsistApp.Bogie> result = emptyList.stream()
